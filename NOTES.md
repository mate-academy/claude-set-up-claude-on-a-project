...
I kept it to four lean parts: a one-line description, commands (`npm run dev`, `npm test`, `npm run lint`), conventions (one route file per resource, all data access through `db/store.js`, the `require.main === module` export pattern that lets tests import `app` directly, and a note to keep CI green before pushing), and an architecture overview of how `server.js`, `routes/`, `db/store.js`, and `tests/` fit together.
...
Started a fresh session and ran `/memory` — `CLAUDE.md` shows as loaded. Ran `/permissions` — the allow, deny, and ask rules above all show up.
